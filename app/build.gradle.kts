...
if (hasGoogleServicesConfig) { apply(plugin = "com.google.gms.google-services"); apply(plugin = "com.google.firebase.crashlytics") }
...
android {
    ...
    defaultConfig {
        ...
        val tidalClientId = localProperties.getProperty("TIDAL_CLIENT_ID")?.takeIf { it.isNotBlank() } ?: System.getenv("TIDAL_CLIENT_ID")?.takeIf { it.isNotBlank() } ?: "nNgez049R98ji742"; buildConfigField("String", "TIDAL_CLIENT_ID", "\"$tidalClientId\"")
        val qobuzAppId = localProperties.getProperty("QOBUZ_APP_ID")?.takeIf { it.isNotBlank() } ?: System.getenv("QOBUZ_APP_ID")?.takeIf { it.isNotBlank() } ?: ""; val qobuzAppSecret = localProperties.getProperty("QOBUZ_APP_SECRET")?.takeIf { it.isNotBlank() } ?: System.getenv("QOBUZ_APP_SECRET")?.takeIf { it.isNotBlank() } ?: ""; buildConfigField("String", "QOBUZ_APP_ID", "\"$qobuzAppId\""); buildConfigField("String", "QOBUZ_APP_SECRET", "\"$qobuzAppSecret\"")
        buildConfigField("String", "LASTFM_API_KEY", "\""); buildConfigField("String", "LASTFM_SECRET", "\"")
        buildConfigField("String", "SUPERPOWERED_LICENSE", "\"$superpoweredLicenseKey\""); buildConfigField("boolean", "SUPERPOWERED_LICENSE_BOUND", "false"); buildConfigField("String", "SUPERPOWERED_LICENSE_CHECK", "\"$superpoweredKeyChecksum\""); val isNightly = project.hasProperty("nightly") && project.property("nightly") == "true"; buildConfigField("Boolean", "IS_NIGHTLY", isNightly.toString()); externalNativeBuild { cmake { cppFlags("-std=c++17", "-fexceptions", "-frtti"); arguments("-DANDROID_STL=c++_shared") } }
    }
    flavorDimensions += listOf("abi", "variant")
    productFlavors { create("foss") { dimension = "variant"; isDefault = true; buildConfigField("Boolean", "CAST_AVAILABLE", "false") }; create("gms") { dimension = "variant"; buildConfigField("Boolean", "CAST_AVAILABLE", "true") }; create("universal") { dimension = "abi"; buildConfigField("String", "ARCHITECTURE", "\"universal\"") }; create("arm64") { dimension = "abi"; ndk { abiFilters += "arm64-v8a" }; buildConfigField("String", "ARCHITECTURE", "\"arm64\"") }; create("armeabi") { dimension = "abi"; ndk { abiFilters += "armeabi-v7a" }; buildConfigField("String", "ARCHITECTURE", "\"armeabi\"") }; create("x86") { dimension = "abi"; ndk { abiFilters += "x86" }; buildConfigField("String", "ARCHITECTURE", "\"x86\"") }; create("x86_64") { dimension = "abi"; ndk { abiFilters += "x86_64" }; buildConfigField("String", "ARCHITECTURE", "\"x86_64\"") } }
...
dependencies {
    testImplementation(libs.junit); "gmsImplementation"(platform("com.google.firebase:firebase-bom:33.1.0")); "gmsImplementation"("com.google.firebase:firebase-analytics"); "gmsImplementation"("com.google.firebase:firebase-crashlytics"); "gmsImplementation"(libs.play.services.auth); "gmsImplementation"(libs.google.api.client.android); "gmsImplementation"(libs.google.api.services.drive) { exclude(group = "org.apache.httpcomponents") }
    ...
    "gmsImplementation"(libs.media3.cast); "gmsImplementation"(libs.mediarouter); "gmsImplementation"(libs.cast.framework); implementation(libs.room.runtime); implementation(libs.kuromoji.ipadic); implementation("com.github.promeg:tinypinyin:2.0.3") { exclude(group = "com.github.promeg.tinypinyin") }; implementation("dev.ffmpegkit-maintained:ffmpeg-kit-full:8.1.7"); ksp(libs.room.compiler); implementation(libs.room.ktx); implementation(libs.apache.lang3); implementation(libs.hilt); implementation(libs.jsoup); ksp(libs.hilt.compiler)
